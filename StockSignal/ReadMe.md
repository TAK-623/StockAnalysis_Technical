...
3. **シグナル抽出**: 条件に合致するブレイクアウト銘柄・AllAbove 銘柄・押し目買い銘柄を抽出
...
python signal_extractor.py --type all_above  # AllAbove のみ
python signal_extractor.py --type push_mark  # 押し目のみ
python signal_extractor.py --type all        # すべて
```
...
- `get_consecutive_tickers()`: 前回結果と今回結果の積集合で連続該当銘柄を特定（ブレイク・AllAbove・押し目の 3 カテゴリ）
- `decorate_company_name()`: 連続該当銘柄の銘柄名先頭に「◎」を付与

**処理順序（重要）:**

1. `main.py` 開始時: 前回の `Breakout.csv`・`AllAbove.csv`・`push_mark.csv` を `Result/Previous/` へコピー
2. `main.py` 本処理: 新しい `Breakout.csv`・`AllAbove.csv`・`push_mark.csv` を生成
...
- **対象銘柄**: `Breakout.csv` / `AllAbove.csv` / `push_mark.csv` 各 CSV の全銘柄（10 銘柄ごとにまとめ画像化）
