"""
株価取得モジュール - 外部API（yfinance）からの株価データ取得

このモジュールは、Yahoo Finance APIラッパーであるyfinanceを使用して、
複数銘柄の株価ヒストリカルデータを外部から取得・ローカル保存します。
ローカルメタデータの読み込みを担当する data_loader.py とは責務を分けています。
