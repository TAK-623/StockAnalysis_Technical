...
**Input.csvの例：**
```csv
Ticker,銘柄名,基準日,機関,目標株価
6946,日本アビオニクス,2025-09-16,野村証券,2500
285A,キオクシア,2025-09-10,大和証券,2800
6141,DMG森精機,2025-06-04,,,
...
[CHART]
# データ取得期間（月数）
# 処理実行時点から過去Nか月分のデータを取得（デフォルト：6か月）
# 処理実行時点（今日）までのデータを取得
...
**取得期間の指定方法：**
- yfinanceの`period`パラメータを使用して期間を指定
- `data_period_months = 6`の場合、`period="6mo"`を指定
- 処理実行時点から過去Nか月分のデータを自動取得

**具体例：**
- 処理実行日: 2024-12-28
- `data_period_months = 6`の場合
- yfinanceで`period="6mo"`を指定
- **実際の取得期間**: 約6か月分のデータ（自動的に最新データまで取得）

**重要：**
- 基準日はチャート上に縦線として表示されるが、データ取得期間には影響しない
- 常に最新の6か月分（設定可能）のデータを取得してチャートを作成
- 基準日はチャート内の任意の位置に表示される可能性がある
- yfinanceのperiodパラメータにより、開始日・終了日の計算が不要
